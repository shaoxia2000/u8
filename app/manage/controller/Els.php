				...
				if ($info) {
					...
					if ($schstype == 2) {
						if (Session::get('school_id') != 0) {
							...
							foreach ($data as $k => $v) {
								db('students' . $pdata['schid'])->where('id_num', trim($v['F']))->update(['chinese' => $v['B'], 'smath' => $v['C'], 'english' => $v['D'], 'zcj' => trim((int)$v['B'] + (int)$v['C'] + (int)$v['D'])]);
							}
							$remsg['code'] = 100;
							$remsg['msg'] = "提交成功";
							return $remsg;
					...
					if ($schstype == 3) {
						...
						foreach ($data as $k => $v) {
							db($dbname)->where('id_num', trim($v['E']))->update(['zcj' => $v['G']]);
						}
						$remsg['code'] = 100;
						$remsg['msg'] = "更新成功";
						return $remsg;
	...
	public function Checkexcelall($data, $str)
	{
		...
		foreach ($datanew as $value) {
			if (!$redis->smembers($key)) {
				...
				if (in_array($value, $dataredis)) {
					array_push($temparr, '身份证号:' . $value . '已经存在！');
				}
			}
		}
		if (count($temparr) > 0) {
