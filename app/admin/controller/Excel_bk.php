...
class Excel extends Common
{
    ...
    {
        $planset = db('planset')->where('schoolid', session('schoolid'))->select();
    ...
    public function test()
    {
        ...
        if ($isstudents) {
            ...
            `house_owner` varchar(15) NOT NULL COMMENT '房屋产权人',
        ...
        if ($isclasses) {
            ...
            `taskid` int(11) NOT NULL,
            `teacherid` int(10) NOT NULL COMMENT '班主任id',
...
//        halt($data);
    ...
    public function fb()
    {
        ...
        //最新的任务id
        if ($schooltype == 2) {
            $data1 = db($studentsdb)->field("id,name,zcj,sex,taskid")->where('sex', '男')->where('taskid', $taskid)->order('zcj desc')->select();
            $res = $this->sort($data1);
            $data2 = db($studentsdb)->field("id,name,zcj,sex,taskid")->where('sex', '女')->where('taskid', $taskid)->order('zcj desc')->select();
            $res = $this->sort($data2);
        } else {
