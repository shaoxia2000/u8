...
class Page {
    ...
    private function first(){
        $html = "";
        if($this->page==1)
            $html.=" <li><a style='magin=10px;' class='current btn disabled'>首 页</a></li>";
        else
            $html.=" <li><a class='btn btn-primary-outline' href='{$this->uri}&page=1'>首 页</a></li>";
    ...
    private function prev(){
        $html = "";
        if($this->page==1)
            $html.=" <li><a class='current btn disabled'>«</a></li>";
        else
            $html.=" <li><a class='btn btn-primary-outline' href='{$this->uri}&page=".($this->page-1)."'>«</a></li>";
    ...
    private function pageList(){
        ...
        for($i=$this->page-$inum;$i<=$this->page+$inum;$i++){
            ...
            if($i == $this->page){
                $linkPage.=" <li><a class='current btn btn-secondary'>{$i}</a></li>";
            }else{
                $linkPage.=" <li><a class='btn btn-primary-outline' href='{$this->uri}&page={$i}'>{$i}</a></li>";
            }
        }
    ...
    private function last(){
        $html = "";
        if($this->page==$this->pageNum)
            $html.=" <li><a class='current btn disabled'>尾 页</a></li>";
        else
            $html.=" <li><a class='btn btn-primary-outline' href='{$this->uri}&page=".($this->pageNum)."'>尾 页</a></li>";
