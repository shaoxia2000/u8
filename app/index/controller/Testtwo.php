...
class Testtwo
{
    ...
    {
        for ($ii = 1; $ii <= 8; $ii++) {
            $dataavg = db('classes4')->where('cno', $ii)->where('taskid', 1)->avg('english');
            $ar[] = $dataavg;
        }
        ...
        $newmax = $max + 1;
        $newmin = $min + 1;
        $maxdata = db('classes4')->where('cno', $newmax)->where('taskid', 1)->order('english', 'desc')->select();
        ...
        for ($iii = 1; $iii <= 8; $iii++) {
            $dataavgnew = db('classes4')->where('cno', $iii)->where('taskid', 1)->avg('english');
            $arr[] = $dataavgnew;
        }
        /*
    ...
    public function sortt($data, $newmin, $pp, $classdb, $taskid)
    {
        ...
        foreach ($data as $k => $v) {
             ...
             */
            $gtitle = $this->getminzcj($v['cid'], $v['chinese'], $v['smath'], $v['english'], $newmin, $pp, $v['sex'], 'classes4', 1);
            $v['cz'] = $gtitle;
            $arr1[] = $v['cz'];
        }
        /*
...
//        halt($arr);
    ...
    public function getminzcj($cid, $chinese, $smath, $english, $newmin, $pp, $sex, $classdb, $taskid)
    {
        static $arr1 = array();
        /*
        ...
        $englishstart = $english - 10;
        $englishend = $english + 10;
...
//        halt($chinesestart.','.$chinesesend);
         ...
         */
        foreach ($data as $k => $v) {
            $v['newzcj'] = $english - $v['english'];
            if ($v['newzcj'] > 0 && $v['chinese'] > $chinesestart && $v['chinese'] < $chinesesend && $v['smath'] > $smathstart && $v['smath'] < $smathend && $v['english'] > $englishstart && $v['english'] < $englishend) {
